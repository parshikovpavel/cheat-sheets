...
Для работы с базами данных используется расширение DBUnit. 

Четыре этапа юнит-тестирования:

\1.     Настройка фикстур (setup)
...
Метод getConnection() используется для возврата соединения с базой данных. Метод getDataSet() задает начальное состояние базы данных перед каждым тестом.
