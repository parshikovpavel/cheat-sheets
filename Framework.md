...
- *Data binding* (связывание данных) – делает данные из *ViewModel* доступными для вывода в виде свойств в *View*. *Data binding* гарантирует, что любые изменения данных в *ViewModel* будут автоматически отражены в *View*.
...
Реактивное приложение удовлетворяет условиям:

- реагирует на действия пользователя – изменением *application state*.
- автоматически уведомляет и перерисовывает View при изменениях *application state*.
