...
Git – распределенная система контроля версий (distributed version-control system, DVCS). Это оз­начает, что после получения из центрального хранилища ветки кодовой базы можно вносить изменения в собственное локальное хранилище, не устанавливая сетевое соединение с сервером центрального хранилища. Клиенты не просто скачивают *snapshot* всех файлов (состояние файлов на определённый момент времени) — они полностью копируют *repository*. Любая копия *repository* является полным бэкапом всех данных.

![dvcs](img/git/dvcs.png)

Чтобы посмотреть историю проекта, Git не нужно соединяться с сервером для её получения и отображения — система просто считывает данные напрямую из локальной *database*. 



Хранение данных в Git похоже на набор *snapshot*'ов файловой системы. Каждый раз, когда вы делаете *commit*, Git запоминает каждый файл в этот момент, и сохраняет *reference* на этот *snapshot* (т.е. ссылку на каждый файл). Для увеличения эффективности, если файлы не были изменены, Git не сохраняет эти файлы заново, а только сохраняет *reference* на предыдущую версию этого файла. Git представляет свои данные как *stream of snapshot*'s (поток).
...
В Git для всего вычисляется *checksum*. Обращение к объектам происходит по этой *checksum*. Git использует *SHA-1 hash*. Это строка длиной в 40 *hexadecimal characters* (0–9, a–f), она вычисляется на основе содержимого файла или структуры каталога. *SHA-1 hash* выглядит так:

```
24b9da6552252987aa493b52f8696cd6d3b00373
```

Git сохраняет все объекты в свою *database* не по имени, а по *hash value* содержимого.

Также в виде *SHA-1 hash* хранится *commit id* (идентификатор *commit*'а).



## Три состояния

В *git* проекте выделяют 3 *section*'s:

![snapshots](img/git/sections.png)

- *working directory* (*working tree*) – один *checkout* (??? снимок, извлечение) одной версии проекта (одного *snapshot*'а). Для этих файлов выполняется *pull* (извлечение) из сжатой *database* в *Git directory*. Файлы помещаются на диск, для того чтобы их можно было использовать или редактировать.
- *Staging area* — это файл, обычно находящийся в *Git directory*, в нём содержится информация о том, что попадёт в следующий *commit*. Технически в *git* он называется — «*index*».
- *Git directory* – место, где Git хранит метаданные и *database* для проекта. Это самая важная часть Git и это та часть, которая копируется при выполнении операции `clone` для *repository* с другого компьютера.

Основной *workflow* при работе с Git выглядит так:

1. Изменяете файлы в *working directory*
2. Некоторые из изменений (*changes*, ??? файлов) добавляем в *staging area*, только эти *change*'s попадут в следующий *commit*.
3. Делаем *commit*, в котором файлы в том виде, как они хранятся в *staging area*, перманентно сохраняются в *snapshot* в *Git directory*.

У Git есть три основных состояния, в которых могут находиться файлы: *modified* (изменён), *staged* (индексирован) и *committed* (зафиксирован):

- *Modified* – файл, для которого был сделан *checkout*, а затем вы его изменили, но ещё не добавили в *staging area*.
- *Staged* — *modified* файл, который добавлен в *staging area* и, тем самым, помечен для включения в следующий *commit snapshot* в его текущей версии.
- *Committed* – файл, сохраненный в *local database* в *Git directory*.
...
Команда `git config` позволяет делать *get* и *set* для *configuration variable*'s. *Configuration variable*'s управляют работой Git.

## Уровни настроек

Можно задавать три уровня настроек (*level*), которые сохраняются в трёх разных местах.

| Уровень      | Описание                                                     | Файл настроек                                   | Команда, позволяющая делать *get* и *set* в указанный файл настроек |
| ------------ | ------------------------------------------------------------ | ----------------------------------------------- | ------------------------------------------------------------ |
| *System*     | Общие настройки для всех пользователей системы и для всех их репозиториев | `/etc/gitconfig`                                | `git config --system`                                        |
| *User*       | Настройки для конкретного пользователя и для всех его репозиториев | `~/.gitconfig`                                  | `git config --global`                                        |
| *Repository* | Настройки конкретного репозитория                            | `/.git/config`, файл `config` в *Git directory* | `git config --local` или просто `git config`. Для исполнения команды необходимо находиться внутри *Git repository* |
...
Если некоторые *name* (названия) настроек указаны несколько раз в файлах разного уровня, то в выводе команды они будут также выведены несколько раз (например, настройки из `/etc/gitconfig` и `~/.gitconfig`). Git использует последнее значение для каждого *name*.
...
Их необходимо задать сразу после установки Git.

Они будут включены в каждый *commit*  и не могут быть изменены в нем.
...
### `core.editor`
 ...
 По умолчанию Git использует стандартный редактор системы, которым обычно является Vim. Если вы хотите использовать другой текстовый редактор, например, Emacs, необходимо его указать:
