...
*Empty interface* может использоваться для ссылки на любое значение (подобно `void*` в C++)
...
*Import path* (путь импорта) – строка, которая однозначно идентифицирует *package* (не файл, а *package*). *Import path* соответствует местоположению *package* внутри *workspace* или в *remote repository* (удаленном репозитории).
...
- *path* к *base directory* – `$GOPATH/src/github.com/user`
- *path* к `hello` *package* – `$GOPATH/src/github.com/user/hello`
...
Здесь указывается *package path* (не файл, а *package*).
...
- используем этот *package* в другом файле `$GOPATH/src/github.com/user/hello/hello.go`.
  ...
  import (
    fmt
    "github.com/user/stringutil" // Тут указываем package path (не файл)
  )
