...
Интерцептор это обёртка вокруг запроса, которая выполняется до и/или после основной бизнес-логики.

Если проще: запрос приходит в сервис, но перед тем как попасть в *handler*, он проходит через цепочку “служебных фильтров”. Такие фильтры могут:

- залогировать запрос и ответ;
- посчитать метрики;
- добавить tracing/span;
- проверить авторизацию;
- преобразовать ошибку;
- сделать retry;
- проставить request id, user id, source и т.п.

В gRPC это обычно выглядит так:

```
client -> interceptor -> handler -> usecase
```

А если интерцепторов несколько:
...
Пример серверных gRPC-интерцепторов:

```
grpcserver.WithUnaryInterceptors(
    grpcmetrics.ErrorCodeInterceptor(),
    grpclogging.UnaryServerInterceptor(logger),
)
```

То есть каждый входящий gRPC-запрос сначала проходит через метрики и логирование, а уже потом попадает в gRPC handler.

Есть похожее понятие `middleware`: это почти то же самое, просто чаще термин используется для HTTP/Kafka. Например Kafka retry middleware оборачивает обработку сообщения и может повторять её при ошибке.
