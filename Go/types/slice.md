...
`slice` (срез) – это дескриптор для непрерывного сегмента *underlying* `array` (базового массива), обеспечивающий доступ к пронумерованной последовательности *element*'ов из этого *array*. Т.е. *slice* – это такой *type*, который может хранить все возможные *slic*'ы для *array* с элементами указанного *type*. 
...
После инициализации *slice* всегда связан с *underlying* `array` (базовым массивом), который содержит его элементы. Таким образом, *slice* разделяет хранилище со своим `array` и с другими `slice`'s того же `array`.
...
Количество *element*'ов называется *length* для *slice* и никогда не бывает отрицательным.*Value* для неинициализированного *slice* равно `nil`.
...
*Length* для *slice* `s` можно узнать с помощью встроенной функции `len()`; в отличие от *array* – *length* для *slice* может меняться во время выполнения. К *element*'ам можно обращаться с помощью *integer index*'ов от `0` до `len(s)-1`. *Index* в *slice* некоторого *element*'а может быть меньше *index*'а TODOD!!!! того же элемента в базовом массиве.
