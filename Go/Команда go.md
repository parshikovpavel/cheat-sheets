...
встроенный в Go инструмент **статического анализа**. Она проверяет исходный код на наличие подозрительных конструкций, которые могут привести к ошибкам во время выполнения программы, но сам компилятор их не ловит.

Проверка *package* в текущей директории.

```bash
go vet
```

Рекурсивно проверяет все пакеты в текущей директории и поддиректориях.

```bash
go vet ./...
```

Проверка конкретного *package*:
...
```bash
go vet путь/к/файлу.go
```

 `go vet` ищет распространённые ошибки и антипаттерны:

- недостижимый код - участки кода, которые никогда не будут выполнены.
- ошибки в строках формата `Printf()`
- и т.д.

Инструмент не проверяет все возможные проблемы и использует ненадежные эвристические методы, поэтому его следует использовать только в качестве ориентира, а не как надежный индикатор корректности программы.

Пример вывода:

```go
$ go vet solution.go 
solution.go:17:14: fmt.Printf format %d has arg "123" of wrong type string
```
...
*Data Race* возникает, когда две *goroutine*'s конкурентно обращаются к одной и той же *variable*, и по крайней мере одно из обращений является записью.

Вот пример состояния *data race*, которое может привести к сбоям и повреждению памяти (`map` не является safe-concurrent):
...
func main() {
	...
	go func() {
		m["1"] = "a" // Первый конфликт доступа
		c <- true
	}()
	m["2"] = "b" // Второй конфликт доступа
	<-c
	for k, v := range m {
		fmt.Println(k, v)
	}
...
Для диагностики подобных ошибок Go включает встроенный *Data Race Detector*. Чтобы использовать его, добавьте флаг `-race`к команде go:

```
$ go run -race mysrc.go // для запуска исходного файла
$ go test -race mypkg // для тестирования package
```
