...
Особенности решения задачи по архитектуре:

- выяснить ограничения в архитектуре (например, сколько серверов, требуемое время ответа, RPS, Request per day). Нужно выяснить все ограничения, т.к. они как правило изначально неизвестны до конца. По каждому ограничению также нужно уточнение. Некоторые ограничения неравномерны: например, Request per day не равномерно распределяются в течении дня ([1](#метрики-для-rps))

При проектировании архитектуры нужно определить:

- объем хранимых данных.

  Для определения объема хранимых данных нужно посчитать:

  ```
  объем_данных = размер_записи * количество_записей_в_день * количество_дней
  ```
...
- *backend*'s для обработки *dynamic requests*'s
...
- база данных вынесена на отдельный сервер. Есть сервер с master DB и несколько slave DB, на которые осуществляется репликация.

- Кеш (memcached, redis) вынесен на отдельный сервер с большим количеством оперативной памяти.
