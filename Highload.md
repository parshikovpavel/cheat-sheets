...
Получив запрос, веб-сервер сначала проверяет наличие ответа в кэше. Если ответ есть, данные возвращаются клиенту. Если нет, то веб-сервер обращается к базе данных, сохраняет ответ в кэше и пересылает его об- ратно клиенту. Эта стратегия называется *кэшем сквозного чтения*. 
...
#### Обновление кэша асинхронно через очередь

Можно не синхронно обновлять кэш, а асинхронно - через очередь.
...
https://habr.com/ru/company/skillfactory/blog/548580/
...
- при работе с БД – к большому объему I/O, т.к. данные не были закэшированы в памяти (например, в *buffer pool*)
...
Работают на уровне L4. 

На них может стоять:

- софт для для фильтрации 
- софт для мониторинга трафика
...
При переходе к распределенной обработке запросов на множестве бекендов архитектура приложения должна быть перестроена. Результат обработки запроса не должен зависеть от того, на каком сервере он обработан (в пределе *stateless*-код).

## Виды архитектуры

- Архитектура с сохранением состояния. От того, хранит сервер состояние или нет, зависит, будет ли он «помнить» данные клиента (состояние) между разными запросами. В большинстве балансировщиков нагрузки для этого предусмотрены *sticky session*

![image-20260716022327140](img/system_design/10.png)

- Чаще используется Архитектура без сохранения состояния.  Отсутствие состояния делает систему более простой, надежной и масштабируемой.
...
- постоянство сессий (*session persistence*) или липкость (*stickiness*).
  ...
  Nginx как балансировщик поддерживает несколько механизмов, позволяющих управлять балансировкой, и отправлять следующий запрос на ту же ноду, что и предыдущий ([1](Webserver.md#методы-балансировки)). Каждый *backend* при этом отвечает за подмножество данных (например, информации о пользователях). :
  - `ip_hash`
  - `hash`
  ...
  ![image-20230318230357920](img/highload/sticky-session.png)

  

  Связано с [MySQL Use case's master-slave репликации](Mysql.md#use-cases-master-slave-репликации)
...
Есть варианты реализации процентного тогла:

- не важно, как пользователи попадают в процентные группы. Просто `rand(100)<=$procent`
- важно, чтобы пользователь попадал постоянно в один процент. Берем остаток от деления `user_id % 100 <= $procent`
