...
Только обратные кавычки могут занимать более одной строки:
...
Для записи многострочных строк в одинарные и двойные кавычках можно использовать:

- экранирование новой строки (*escaping*):
...
`indexOf()`  ищет подстроку `substr` в строке `str`, начиная с позиции `pos`, и возвращает позицию, на которой располагается совпадение, либо `-1` при отсутствии совпадений.
...
Метод выполняет указанную функцию один раз для каждого элемента в массиве.
...
- `callback` – функция, которая будет вызвана для каждого элемента массива. Ее аргументы:
...
- `event.stopImmediatePropagation()` – остановить всплытие и не выполнять другие обработчики на данном элементе
- `event.preventDefault()` – отменить действие браузера по событию по умолчанию. Например, отменить переключение состояния `checkbox` по щелчку. При этом распространение (всплытие) событие продолжает как обычно.
