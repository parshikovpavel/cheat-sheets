...
Остановилсяhttps://jekyllrb.com/docs/front-matter/
...
Для того чтобы *Jekyll* обрабатывал файл (*object*'ы, *tag*'и, *filter*'ы), в начале файла должен быть размещен *front matter* (возможно пустой). Однако *post*'ы обрабатываются всегда, даже если не содержат *front matter* (точно??? написано что у них всегда должен быть *front matter*).
