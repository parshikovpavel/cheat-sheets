...
Является одной из функций шардирования  [1](Sharding.md#общие-принципы)

Общий объем кэша будет равен сумме объемов кэшей всех memcached, входящих в кластер. Процесс memcached может быть запущен на сервере, где слабо используется процессор и не загружена до предела сеть.
...
Для добавления серверов используется функция `addServer`. В таком случае, Вы просто указываете набор серверов для соединения, а платформа сделает все остальное. Memcached сам умеет определять нужный сервер для каждого ключа на основе алгоритма *consistent hashing*.
...
По сути при этом реализуется горизонтальное шардирование ключей на уровне *memcached*. 

Более подробно [1](Sharding.md#консистентное-кеширование)
...
Чтобы критическая секция гарантированно завершилась при завершении работы скрипта, можно окончание критической секции поместить в `register_shutdown_function()`:

```php
CriticalSection::one('key', 20);
register_shutdown_function(function() {
    CriticalSection::end('key');
});
```
