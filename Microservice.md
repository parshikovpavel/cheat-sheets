...
Все современные крупные компании – Amazon, Netflix, ebay – стартовали как монолит.
...
- низкая зацепленность и возможность быстрого внедрения новых технологий
- Отказоустойчивость. легкая Graceful degradation. Если упало что-то одно, то не падает все.
...
- много репозиториев, много сервисов, требуется платформа для их общего контроля – подъем версий Go, версии библиотек, количество используемых ресурсов
- проблемы с консистентностью данных
- овер на сетевые походы 
- мало инструментов для разработки
- нужно быть готовым к недоступности части сервисов partition tolerance
...
Если сервиса часто взаимодействуют – они сильно зацеплены и мы поличили распределенный монолит
...
А внешний frone-end сервис (api-composition) должен просто перепрокинуть ответ из внутреннего сервиса
