...
* Коллекции состоят из *документов* (аналог строки). *Документ* хранит информацию о полях, которые находятся внутри него. Разные *документы* могут иметь различные наборы полей.  Невозможно автоматически изменить структуру всех документов сразу, при необходимости нужно изменять структуру каждого документа по отдельности. Документ описывается в форме JSON объекта.
...
* Ограниченные коллекции (capped collection)
