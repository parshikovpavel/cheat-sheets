...
Фильтр по документам, что `tags` содержит элемент `"red"`, возможно, с наличием других элементов:
...
Аналогично можно выполнить проверку вхождения в массив документа, проверяется точное совпадение, включая порядок полей. В примере, в массиве будет искаться точно такой документ.
...
Для одиночного query operator, достаточно, чтобы хотя бы один элемент массива удовлетворял условию. Например, проверить, что хотя бы один элемент массива меньше 200:
...
При обычном задании множественного условия, разные части этого условия могут удовлетворяться разными элементами массива. Например, проверить, что:

- хотя бы один элемент больше 15
- хотя бы один элемент меньше 20 (возможно это один и тот же элемент или разные).
...
Если требуется, чтобы хотя бы один элемент массива удовлетворял всем критериям условия, следует использовать оператор [`$elemMatch`](#elemmatch). Например, найти массивы, где хотя бы один элемент удовлетворяет сразу обоим критериям: больше 15 и меньше 20.
...
##### Условие на элемент массива с определенным индексом

Используя [точечную запись](#массивы-и-вложенные-документы), можем накладывать условия на элемент массива с определенным индексом. Например, условие на второй (`id=1`) элемент массива:

```json
{ "dim_cm.1": { $gt: 25 } }
```

Аналогично, можно наложить условие на вложенный (*nested*) объект внутри элемента массива с определенным индексом. Например, условие, что массив имеет в качестве `id=0`  документ, содержащий поле `qty<=20`:

```json
{ 'instock.0.qty': { $lte: 20 } }
```

##### Фильтр по массивам документов (*nested*)

###### Эквивалентность поля вложенных документов

Найти те записи, где есть вложенные документы с указанным значением поля. Например, найти записи, где есть вложенные в массив документы с `qty<=20`:
