...
[Павел](https://vk.com/id2733257) [9:34](https://vk.com/im?sel=2733257&msgid=57660)
...
[Павел](https://vk.com/id2733257) [11:52](https://vk.com/im?sel=2733257&msgid=57662)
...
[Павел](https://vk.com/id2733257) [12:10](https://vk.com/im?sel=2733257&msgid=57669)
...
[Павел](https://vk.com/id2733257) [12:28](https://vk.com/im?sel=2733257&msgid=57670)
...
ры'' отличаются от предыдущих пяти концепций, но они очень важны (хотя порой их обходят вниманием) и заслуживают отдельного обсуждения. Важно понимать, что когда мы запрашиваем у MongoDB какие-либо данные, то она возвращает курсор, с которыми мы можем делать все что угодно - подсчитывать, пропускать определенное число предшествующих записей - при этом не загружая сами данные.

[     ](https://vk.com/id2733257)

[Павел](https://vk.com/id2733257) [12:37](https://vk.com/im?sel=2733257&msgid=57672)

 

''Курсоры'' отличаются от предыдущих пяти концепций, но они очень важны (хотя 
...
[Павел](https://vk.com/id2733257) [12:55](https://vk.com/im?sel=2733257&msgid=57673)
...
''Курсоры'' отличаются от предыдущих пяти концепций, но они очень важны (хотя 
 порой их обходят вниманием) и заслуживают отдельного обсуждения. Важно понимать, 
 что когда мы запрашиваем у MongoDB какие-либо данные, то она возвращает курсор, 
 с которыми мы можем делать все что угодно- подсчитывать, пропускать

[     ](https://vk.com/id2733257)

[Павел](https://vk.com/id2733257) [13:45](https://vk.com/im?sel=2733257&msgid=57675)

 

Иногда требуется развернуть базу данных в другой СУБД, в СУБД MySQL более ранней версии, не поддерживающей нововведения поздних версий, или на сервере, где отсутствует доступ к каталогу данных. В этом случае часто прибегают к созданию SQL- дампов. SQL-дамп — это текстовый файл с SQL-инструкциями, выполнение кото

[     ](https://vk.com/id2733257)

[Павел](https://vk.com/id2733257) [19:03](https://vk.com/im?sel=2733257&msgid=57727)
...
Коллекции состоят из нуля или более ''документов''. Опять же, документ можно рассматривать как ''строку''.4. Документ состоит из одного или более ''полей'', которые - как можно догадаться - подобны ''колонкам''.5. ''Индексы'' в MongoDB почти идентичны таковым в реляционных базах данных.

[     ](https://vk.com/id2733257)

[Павел](https://vk.com/id2733257) [19:13](https://vk.com/im?sel=2733257&msgid=57729)
...
[Павел](https://vk.com/id2733257) [19:33](https://vk.com/im?sel=2733257&msgid=57745)

 

термины, хоть и близки своим ''реляционным'' аналогам, но не полностью идентичны им. Основное различие в том, что реляционные базы данных определяют ''колонки'' на уровне ''таблицы'', в то время какдокумент-ориентированные базы данных определяют ''поля'' на уровне ''документа''. Это значит, что любой документ внутри коллекции может иметь свой собственный уникальный набор полей

[     ](https://vk.com/id2733257)

[Павел](https://vk.com/id2733257) [23:00](https://vk.com/im?sel=2733257&msgid=57748)

 

несколько глобальных команд, например help или exit. Команды, которые вы запускаете применительно к текущей базе данных исполняются у объекта db, например [db.help](https://vk.com/away.php?to=http%3A%2F%2Fdb.help&cc_key=)() или db.stats() . Команды, которые вы запускаете применительно к конкретной коллекции, исполняются у объекта db.имя_К0ЛЛЕКЦИИ, например [db.unicorns.help](https://vk.com/away.php?to=http%3A%2F%2Fdb.unicorns.help&cc_key=)() или db.unicorns.count() .

 

[db.help](https://vk.com/away.php?to=http%3A%2F%2Fdb.help&cc_key=)() и получите список команд, которые можно выполнить у объекта db.

 

консоль интерпретирует JavaScript, если вы попытаетесь выполнить метод без скобок, то в ответ получите тело метода, но он не выполнится.Не удивляйтесь, увидев function (...){,

 

выбора базы данных воспользуемся глобальным методом use

 

введите use learn. Неважно, что база данных пока еще не существует. В момент создания первой коллекции создастся база данных learn

[     ](https://vk.com/id2733257)

[Павел](https://vk.com/id2733257) [23:13](https://vk.com/im?sel=2733257&msgid=57753)

 

еперь, когда вы внутри базы данных, можно вызывать у неё команды, например db.getCollectionNames(). В ответ увидите пустой массив ([ ]). Поскольку коллекции бесструктурны (в оригинале ''schema-less". Здесь и далее - прим. перев.), мы не обязаны создавать их явно. Мы просто можем вставить документ в новую коллекцию. Чтобы это сделать, используйте команду insert, передав ей вставляемый документ:db.unicorns.insert({name: 'Aurora', gender: 'f', weight: 450})

 

ет метод insert (''вставить'') в

 

system.indexes создается в каждой базе данных и содержит в себе информацию об индексах этой базы.
