...
В Linux *socket* — это программный *endpoint*, который позволяет процессам (программам) обмениваться данными — как внутри одного компьютера, так и по сети. По сути, это абстракция, которая даёт программам удобный интерфейс для работы с сетевыми протоколами, скрывая от них низкоуровневые детали передачи данных.

![Передача данных через UNIX сокеты](img/go/1.png)

У сокета есть несколько ключевых характеристик:

- Домен (Domain).  Определяет семейство протоколов и пространство адресов. Например, `AF_INET`  для для сетевых сокетов (TCP/IP), `AF_UNIX` для Unix-доменных сокетов (для межпроцессного взаимодействия на одном компьютере). 
- Тип (Type). Например, `SOCK_STREAM` - потоковый сокет, обеспечивает надёжную, упорядоченную передачу данных (как TCP). `SOCK_DGRAM` - датаграммный, для ненадёжной, но быстрой передачи (как UDP). И другие.
- Адрес. Для *network socket*'ов это комбинация IP-адреса и номера порта. Для Unix-доменных сокетов адресом выступает путь в файловой системе.

Порядок использования *socket*'ов:

1. Создание через `socket()`, который создаёт сокет и возвращает его дескриптор.
2. Сервер привязывает сокет к локальному адресу (например, порту) с помощью `bind()`. 
3. Сервер вызывает  `listen()` , чтобы перевести сокет в режим прослушивания
4. Клиент использует `connect()` для инициации соединения с сервером.
5. Данные отправляются и принимаются через `send()`/`receive`
6. Когда обмен закончен, сокет закрывается.
...
Существует несколько методов мониторинга сетевых событий:
...
Ядро блокирует процесс и следит за состоянием. Как только хотя бы один дескриптор становится «готовым», `select()` возвращает управление. 

Когда проектировался `select()` никто не ожидал, что в будущем у нас появится необходимость писать многопоточные приложения, обслуживающие тысячи соединений. У select() есть сразу несколько существенных недостатков, делающих его плохо пригодным для работы в такого рода системах:

- Есть жёсткий лимит `FD_SETSIZE` (обычно 1024).  
- Ядро при каждом вызове заново проверяет весь диапазон дескрипторов, даже если активно всего несколько. Сложность O(N).
- `select()` модифицирует переданные структуры `fd_set` — их нельзя переиспользовать без переинициализации. 
- Чтобы понять, какие именно дескрипторы сработали, нужно вручную перебирать все и проверять `FD_ISSET`

## `poll()`

`poll()` — это более новый метод опроса сокетов, созданный после того, как люди начали пытаться писать большие и высоконагруженные сетевые сервисы. Он спроектирован намного лучше и не страдает от большинства недостатков метода select. При написании современных приложений вы будете выбирать между использованием poll и epoll/libevent.
...
Массив `fds` содержит структуры, где для каждого дескриптора задаётся, какие события интересны (`events`) и какие реально произошли (`revents`). 

Преимущества `poll()` перед `select()`:
...
У poll есть недостатки:

- Как и при использовании `select`, невозможно определить какие именно дескрипторы сгенерировали события без полного прохода по всем наблюдаемым структурам и проверки в них поля `revents` (сложность O(N))

Однако, это несущественно для большинства клиентских приложений, за исключением p2p протоколов, где каждый из клиентов может быть связан с тысячами других. 

`poll` является более предпочтительным даже по сравнению с более современным epoll если:
...
`epoll()` — это новейший и лучший метод ожидания событий в Linux (и только в Linux). От `poll` и `select` он отличается тем, что предоставляет API для добавления/удаления/модификации списка наблюдаемых дескрипторов и событий.
...
- epoll возвращает список только тех дескрипторов, для которых реально произошли наблюдаемые события. Не нужно просматривать тысячи структур в поисках той, возможно, одной, где сработало ожидаемое событие. (Сложность O(1) )
...
- Ваше приложение использует пул потоков для обработки сетевых соединений. Выигрыш от epoll в однопоточном приложении будет ничтожен, не стоит и заморачиваться на реализацию.Вы ожидаете относительно большого числа соединений (от 1000 и выше). На небольшом количестве наблюдаемых сокетов epoll не даст прироста производительности, а если сокетов буквально несколько штук — может даже замедлить.
...
В этих механизмах вы передаёте ядру **список дескрипторов**, за которыми хотите следить:

- В `select` — три битовые маски (`readfds`, `writefds`, `exceptfds`).
- В `poll` — массив структур `struct pollfd`, где у каждой есть поле `fd` (сам дескриптор) и флаги событий.
- В `epoll` — регистрируете дескрипторы через `epoll_ctl`, а потом ждёте события на них через `epoll_wait`.

Ядро говорит: «дескриптор № 7 готов к чтению» или «на дескрипторе № 12 есть данные». Процесс по этому номеру делает `read(7, ...)` и т. д.

```c
int sock = socket(AF_INET, SOCK_STREAM, 0);  // получил дескриптор, например, 3
int fd = open("data.txt", O_RDONLY);        // получил дескриптор, например, 4

// Хотим ждать события сразу на обоих:
struct pollfd fds[2];
fds[0].fd = sock;
fds[0].events = POLLIN;
fds[1].fd = fd;
...
poll(fds, 2, -1);  // ждём, пока хоть один из дескрипторов станет готов
```
