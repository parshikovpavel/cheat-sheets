...
- Строки, содержащие целое число (исключая случаи, когда число предваряется знаком `+`) будут преобразованы к типу `int`
...
- к `private` свойствам спереди будет дописано имя класса, к `private` свойствам спереди будет добавлен символ `*`. Эти добавленные значения с обоих сторон заключаются в нулевые байты.
...
Для `iterable` реализован [LSP](), дочерние классы:
...
<u>Преобразование `array` в `object`</u>

При преобразовании `array` в `object` будет  создан объект:

- свойства объекта – ключи массива
- значения свойств – значения массива
- числовые ключи не будут доступны пока не проитерирован объект (может поправлено?). 

Для доступа к свойству с числовым именем (например, полученном при преобразовании `array` в `object`) нужно использовать фигурные скобки:
...
<u>Преобразование скалярных типов в `object`</u>
