...
- объявить ее глобальной внутри определения функции:
...
| `array`                                                | `array`                                                | `array` с меньшим числом элементов считаются меньше. Если число элементов в `array`'s одинаковое, то сравниваются значения соответствующих ключей в порядке ключей первого `array`. Если при сравнении ключ из первого `array` не найден во втором – возвращается `null` (алгоритм записан ниже) |
...
Следующие значения аргумента `$var`, которые удовлетворяют сравнению `$var == false`, воспринимаются как пустые:
...
Функции можно вызывать через конструкции, относящиеся к типу `callable`. Подробнее о типе [`callable`](#callable). Возможны два способа обращения к функции через `callable`:
...
class A {} //класс
$a = new A;
$func_bind = $func->bindTo($a, $a); //привязка к классу и его области видимости
$func_bind(1); # object(A)#2 (0) { }
```

или можно через статическую функцию `bind` класса `Closure`

```php
$func_bind = Closure::bind($func,$a, $a);
```

Существует также функция `Closure::call()`, позволяет как в *javascript*, не создавать новый `Closure` через `bindTo`, а сразу вызвать анонимную функцию с привязкой объекта к `$this`:
