...
В PHP не существует оператора деления целых чисел. Если деление выполняется нацело, то результат `int`, если нет – `float`. Если привести значение к `int`, оно будет округлено вниз, то есть будет отброшена дробная часть числа. Можно использовать функции `floor` (в меньшую сторону) и `ceil` ( в большую).
...
Если оператор или функция требуют `int`, то значение будет автоматически преобразовано в `int`.
...
Попытка записи в смещение за границами строки дополнит строку пробелами до этого смещения. При использовании в качестве смещения `float`, оно будет преобразовано в `int`. Смещение в строке должно задаваться либо целым числом, либо строкой, содержащей цифры, иначе будет выдаваться предупреждение. Ранее смещение, заданное строкой вида `string("foo")`, без предупреждений преобразовывалось в `0`.
