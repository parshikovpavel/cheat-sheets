...
- обе переменные изначально ссылаются на один `zval`
- `refcount++` инкрементируется.  

При изменении значения одной из этих переменных, PHP обнаружит, что `refcount > 1` и выполнит:

-  скопирует `zval` и сделает изменения там
- изменяемая переменная связывается с новым `zval`.
