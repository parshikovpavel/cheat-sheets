...
С PHP7.3 можно ставить запятую `,` после последнего аргумента.
...
Начиная с PHP 7.4.0, определение *property* может включать объявление типа, за исключением типа `callable`
...
2. Имеются следующие отличия, допустимые согласно *LSP*:
   ...
   - `B` сужает область возвращаемых значений `A` (постусловия могут быть усилены в подклассах):

     ```php
     function foo(int $x): int;
     ```

     Не допускается сужать область значений, указанием дочернего класса:
     ...
     class ChildB extends ParentB {
         # Fatal error: Declaration of ChildB::a(): ChildA must be compatible with ParentB::a(): ParentA
         function a() : ChildA {}
     }
     ```

3. Возможна комбинация из нескольких отличий:
