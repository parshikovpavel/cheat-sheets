...
- `о` (деление). Возвращает результат типа:
...
Ковариантность для *typed property*'s в дочерних классах не поддерживаются.
...
Блок `finally` выполняется каждый всегда, независимо от ошибок, исключений или даже `return` операторов внутри блоков `try` или `catch`. Однако `finally` не сработает, если `try` или `catch` выполнят `die()` / `exit()` .

Например здесь функция `hi()` будет вызвана, в отличии от случае, если бы `hi()` был бы за пределами `finally {}`
...
Частый *case* использования `finally` – закрытие соединения с базой данных в долго работающем *worker*'е. Необходимо закрывать соединение, даже при возникновении необработанного *exception* или *error* (которые могут вызывать даже остановку скрипта)
...
Одно из распространенных применений, которое я вижу, - это закрытие соединения с базой данных в долго работающем работнике - вы хотите, чтобы это происходило каждый раз (с исключением или без), чтобы у вас не было зависшего соединения, которое не позволяет серверу базы данных принимать новые соединения. .
...
Здесь мы всегда будем закрывать соединение с базой данных. Если это обычный запрос, мы закрываем соединение после успеха, и скрипт продолжит выполнение.

Если это ошибочный запрос, мы все равно закрываем после того, как исключение было выброшено, а неперехваченное исключение вызовет остановку скрипта.
...
###### Создание и удаление ссылки на `zval`
...
При присваивании переменной-ссылке другой ссылки, переменная-ссылка перепривязывается на другую структуру `zend_reference`. Т.е. не важно, была переменная раньше ссылкой или нет, переменная полностью переинициализируется. Т.е. нет возможности через одну ссылку на  `zend_reference` повлиять на привязку других ссылок.
...
`WeakReference` (слабая ссылка) позволяет программисту получить такую ссылку на объект `$object`, которая не влияет на количество ссылок на него, поэтому сборщик мусора сможет освободить этот объект. Аналог распределенных систем кеширования типа `memcached` с вытеснением из кеша. Могут использоваться для реализации кеша в рамках *request*'а.
...
Перенос строки внутрь скобок можно вставить так:
...
Найти название тега из одного слова:
...
Возвращает строку из одного символа, код которого задан аргументом `$bytevalue`

```php
chr ( int $bytevalue ) : string
```

Возвращает символ только для однобайтовой кодировки. 

### `ord`

Конвертирует первый байт строки `$string` в число от 0 до 255.
...
Для работы с MySQL в PHP используется драйвер  `mysqlnd` (*native driver*). `mysqlnd` создавался специально для PHP систем, у него есть преимущество в скорости и расходах памяти перед `libmysqlclient`. Поэтому его использование предпочтительней. Он предоставляет 3 API: 
...
`mysqli` на 6% быстрее `PDO` и поддерживает дополнительные фичи, например, асинхронные запросы. Поддерживаются процедурная и объектно-ориентированная парадигмы. 
...
Функция `mysqli_query()` использует текстовый протокол, т.е. запрос отправляется в текстовом виде и все данные в результирующем наборе также преобразуются в текстовые строки. 

Можно использовать режимы:

- `MYSQLI_STORE_RESULT` – буферизированный результат 
...
$stmt->execute();
$res = $stmt->get_result();
var_dump($res->fetch_assoc()); # id = int(1)
```
...
- `$types` –  строка, где каждый символ определяет тип переменной из списка аргументов `$var1`, `$...`. Длина строки `$types` должна в точности соответствовать количеству переменных.
...
- `$var1`, `$...` – переменные, которые привязываются. 
...
Входные параметры указываются внутри предложения `CALL`.
...
В режиме *CGI* (но не *FastCGI* наверное?) PHP-интерпретатор создается и уничтожается при каждом запуске скрипта, поэтому все используемые им ресурсы закрываются и, следовательно, постоянные соединения отсутствуют. 

Параметры `php.ini` для настройки постоянных соединений: 

- `mysqli.allow_persistent`  – включение механизма постоянных соединений
- `mysqli.max_links` – максимальное количество соединений, которые может открыть один скрипт
- `mysqli.max_persistent` – максимальное количество соединений, которые может открыть один PHP процесс (веб-сервер может порождать множество PHP процессов).
...
Термин пул соединений в `mysqli` означает поддержку постоянных соединений.  

#### Асинхронные запросы

Асинхронные запросы имеют смысл, если в проекте используете шардирование данных на уровне бизнес-логики и возникает необходимость сделать 10 одинаковых запросов к 10 разным серверам
...
- количество готовых к работе соединений.
...
`PDO` предоставляет API интерфейс взаимодействия с базой данных, не зависящий от конкретной СУБД. Можно поменять сервер баз данных, например с *Firebird* на *MySQL*, и это приведет лишь к незначительным изменениям в PHP коде. 

`PDO` не поддерживает следующие возможности MySQL:
...
- `flags` – флаги, возможные значения:
  - `"a"` (сегмент ранее создан, чтение)
  - `"c"` (создание сегмента, чтение/запись)
  - `"w"` (чтение/запись, чаще всего)
  - `"n"` (создание нового сегмента, если существует – отказ). 
...
Также функции `shmop_close()` – закрытие блока, `shmop_delete()` – удаление блока.
...
Семафор – счетчик доступа к публичным ресурсам. Используется для работы с разделяемой памятью. 
...
- `$max_acquire` – число процессов, которые могут одновременно использовать семафор
...
- `$nowait` – ждать захвата семафора или нет. Если не ждать, то вернет `false` когда не может быть захвачен.
...
Записать или изменить значение:

```php
bool shm_put_var ( resource $shm_identifier , int $variable_key , mixed $variable )
```

- `$variable_key` -  ключ для переменной
- `$variable` – поддерживаются все переменные допускающие сериализацию
...
Нужно в *curl* задать несколько адресов, потом запустить мультизапрос:

```php
int curl_multi_exec ( resource $mh , int &$running )
```

Блокировать выполнение скрипта, пока какое-либо из соединений не станет активным:
...
- `PATHINFO_FILENAME` – название файла без расширения
...
- `real_usage`:
  - `true` – реальное количество памяти, выделенной PHP скрипту, включая неиспользуемые страницы
  - `false` – количество используемой памяти.
...
- `$get_as_float` – вернуть `float` вместо `string`
...
Задает время в секундах, в течение которого скрипт должен завершить работу. Если скрипт не успевает, вызывается фатальная ошибка. По умолчанию дается 30 секунд, либо время, записанное в настройке `max_execution_time` в `php.ini`.
...
Если параметр `$object` не указан – возвращается имя класса, где записана функция (т.е. без LSB).
...
Проверяет заданное *assertion* (утверждение) и совершит соответствующее действие, если результатом проверки окажется `false`.
...
-  `$assertion` – проверяемое утверждение. Типы:
  - `string` – deprecated. 
  - `bool` 
...
В PHP 7 `assert()` - это языковая конструкция, позволяющая определять *expectation* (ожидания) – *assertion*, которые работают в среде разработки и тестирования, но в целях оптимизации отключены на продуктовом окружении.
...
| ------------------ | ------------------------- | ------------------------------------------------------------ |
| `zend.assertions`  | `1`                       | `1` – генерирует и выполняет код (*dev*)<br/> `0` – генерирует код, но   перепрыгивает через него во время выполнения<br/> `-1` – не генерирует код (*prod*) |
| `assert.exception` | `0`                       | `1` – выбрасывает исключение<br/>`0` – генерирует предупреждение |
...
- `zend.assertions=1` и `assert.exception=0`:

  ```
  Warning: assert(): assert(false) failed in - on line 2 Hi!
  ```

- `zend.assertions=1` и `assert.exception=1`:
...
1. Для отправки кода состояния HTTP, начинается с `HTTP/`  

   ```php
   header("HTTP/1.0 404 Not Found");
   ```

2. Заголовок  `Location:`. В этом случае функция не только отправляет этот заголовок браузеру, но также возвращает ему код состояния `302 Moved Permanently`, если ранее не был установлен код `201` или `3xx`.
...
Функции `die()` и `exit()` эквивалентны и являются псевдонимами друг друга. Именование `exit()` взято из C, `die()` из Perl.
...
Если `$status` задан в виде `string`, то эта функция выведет содержимое `$status` перед выходом. Если `$status` задан в виде `int`, то это значение будет использовано как статус выхода и не будет выведено (должны быть в диапазоне от 0 до 254, статус 0 – успешное завершение). 

# Архитектура запуска

## Способы ускорения запуска

- `php-fpm` – экономия ресурсов на запуск интерпретатора PHP, использование протокола `fast-cgi`
- `ReactPHP` – позволяет избежать накладных расходов на *bootstrap*, производя его один раз при старте приложения.
...
- *Zend OPcache.* Сохраняет скомпилированный байт-код в разделяемой памяти. Включен по умолчанию начиная с PHP 5.5. Проект развивается. Многие оптимизации (вывод типов, JIT) являются частью *Zend OPcache* и без него не работают.
...
Про модели мультипроцессинга *Apache* смотреть [1](Webserver.md#модели-мультипроцессорности)
...
- В другие сервера (*nginx*) PHP встроить нельзя. Веб-сервер взаимодействует с внешними процессами PHP через FastCGI, *Thread safety* значения не имеет. 
...
KPHP (kPHP, KittenPHP) —транслятор PHP-кода в С++ от компании ВКонтакте. Приставка К- происходит от слова *kitten* (котёнок), что стало своеобразной традицией во внутренних разработках ВКонтакте.
...
- Лексический анализ – последовательность символов преобразуется в последовательность лексем (например, ИМЯ «variable», ПРИСВАИВАНИЕ, СЛОЖЕНИЕ).


- Синтаксический (грамматический) анализ (*parsing*, па́рсинг) – процесс сопоставления линейной последовательности лексем (слов, токенов) с формальной грамматикой. Результатом является дерево разбора (синтаксическое дерево). 

  ![доступ к zval](https://parshikovpavel.github.io/img/php/parsing.png)

   

- Семантический анализ – дерево разбора обрабатывается с целью установления его семантики (смысла) — например, проверка ошибок типа,  привязка объектов (связывание ссылок переменных и функций с их определениями), определение типов выражений, выбрасывание исключений и предупреждений. Результат может быть дополненным деревом разбора или чем-то ещё удобным для дальнейшей обработки. 
...
JIT – комбинация AOT и интерпретации. 
...
1. Приложение компилируется в байт-код для исполнения под виртуальной машиной (*bytecode compilation*). На этом этапе выполняется большинство тяжеловесных операций: парсинг исходного кода и выполнение базовых оптимизаций.
...
Реализация структуры данных *Heap* (куча, [1](Algorithm.md#heap)) в виде бинарного дерева, каждый узел которого имеет не более двух дочерних узлов. Это абстрактный класс, требующий расширения с определением метода `compare()`, позволяющего выполнять сортировку в реальном времени при вставке новых узлов в дерево. Каждый узел больше или равен своим потомкам, метод `compare()` используется для сравнения и является общим для всей кучи. 
...
{
    /**
     * Реализация абстрактного метода compare для сравнения значений
...
abstract SplHeap implements Iterator , Countable {
    ...
    public count ( void ) : int
    public current ( void ) : mixed
    public extract ( void ) : mixed     # извлекает узел из heap и пересортирует ее
...
Реализация структуры данных *Priority queue* (приоритетная очередь, [1](Algorithm.md#priority-queue)) при помощи *Heap* с максимальным элементом наверху (`SplMaxHeap`).
...
Массив, похожий на обычный `array`, но с ограничениями:

- имеет фиксированный размер `$size`
- в качестве ключей могут быть только целые числа `0 ... $size-1`

Это позволяет хранить данные в непрерывной области памяти и доступ к элементу осуществляется по индексу со смещением. Длина может быть изменена, но это затратная операция. 

По тестам скорость работы выше на 20-50% чем при работе с обычными массивами. 
...
Структура для хранения объектов, причем объекты рассматриваются как ключи (т.е. это вроде `array`, который позволяет использовать объекты в качестве ключей).
...
- просто хранить объекты как *key*'s и не привязывать к ним никаких *value*'s:
...
- хранить объекты как *key*'s и привязывать к ним *value*'s:
...
- проверка, соответствует ли пароль хешу:
...
![password hash](https://parshikovpavel.github.io/img/php/passwordhash.png)

   

## Контроль входных данных

Необходим, если данные получены из неизвестных или ненадежных источников, таких, как пользовательский ввод. Например, такие данные можно получить из HTML-форм.

### Функции `ctype_*`

Функции `ctype_*` проверяют, что все символы строки удовлетворяют какому-то условию. Поддерживаются только однобайтные ASCII-символы с кодами в диапазоне `0 - 255` (т.е. можно использовать с кодировкой Win1251, но нельзя с UTF8). Учитывает текущие региональные установки, управление которыми возможно через `setlocale()`.

Работают быстрее регулярных выражений и функций `str_*` и `is_*`, т.к.  использует родную библиотеку C и сравнивают коды символов.

Примеры функций:

- Проверить, что все символы в строке `$text` являются буквами или цифрами:

  ```php
  ctype_alnum ( string $text ) : bool
  ```

- Проверить, что все символы в строке `$text` являются буквами:

  ```php
  ctype_alpha ( string $text ) : bool
  ```

- Проверить, что все символы в строке `$text` являются буквами в нижнем регистре:
...
- <ins>Проверка</ins>. Используется для определения соответствия данных определённым критериям. Например, применение `FILTER_VALIDATE_EMAIL` позволяет проверить, являются ли введённые данные адресом *email*, однако, сами данные при этом останутся нетронутыми.

- <ins>Очистка</ins>. Используется для извлечения из данных нежелательных конструкций. Например, применение `FILTER_SANITIZE_EMAIL` удалит все символы, которые не должен содержать *email*-адрес. То есть, проверки данных не происходит.

Кроме того, можно использовать различные флаги, как для проверки, так и для очистки, для более тонкой настройки. Например, передача флага `FILTER_FLAG_SCHEME_REQUIRED` при фильтрации URL установит необходимым условием наличие протокола (например, `http://` в `http://example.org/foo`).

<ins>Примеры фильтров проверки</ins>:
...
- `$options` – ассоциативный массив параметров
...
$options = array(
    'options' => array(
        ...
        'min_range' => 0  # минимально допустимое значение
    ),
    'flags' => FILTER_FLAG_ALLOW_OCTAL, # разрешить восьмиричные числа
);

var_dump(filter_var('-1', FILTER_VALIDATE_INT, $options));  # int(3)
var_dump(filter_var('010', FILTER_VALIDATE_INT, $options)); # int(8)
```

### Обработка текста

При обработке текста, полученного от пользователя, необходимо выбрать один из двух вариантов:

- Текст не рассматривается, как HTML. 

  Обработка простая: экранировать опасные символы и вывести его в виде обычного текста.

- Текст рассматривает как HTML.

  Лучше всего воспользоваться библиотекой `HtmlPurifier`.

#### Другие подходы к контролю

- для дат – класс `DateTime`
...
Параметры в *prepared statement* не нуждаются в экранировании, так как никогда не подставляются непосредственно в строку запроса.

Поддерживается в расширениях:

- `mysqli` ([1](#prepared-statement-mysqli))
- `PDO`

# Работа с файлами и каталогами

Особенности путей в PHP:

- PHP прозрачно преобразует прямые `/` и обратные `\` слеш в нужные для текущей операционной системы (Windows или UNIX). Поэтому не важно, какой слеш использовать. 
- Можно работать с файлами на удаленном сервере, как будто они лежат локально. Если вы предваряете имя файла строкой `http://` или `ftp://`, то РНР понимает, что нужно на самом деле установить сетевое соединение и работать именно с ним, а не с файлом. При этом в программе такой файл ничем не отличается от обычного.

## Text и binary mode's

Для разделения строк ОС используют ([1](TextEncoding.md#перевод-строки)):

- UNIX, UNIX-подобные операционные системы (GNU/Linux, Mac OS X, FreeBSD и др.) – `\n`
- Microsoft Windows – `\r\n`

Существует два режима работы с файлами:

- текстовый (text) `t`. Имеет особенное поведение в Windows, при работе с текстовыми файлами `\г\п` прозрачно для скрипта преобразуется в  `\n`. В результате при чтении текстового файла в строку, длина такой строки будет меньше физического размера файла, т.к. из нее "исчезли" символы `\г`.  
- бинарный (binary, binary safe) `b` ([1](#binary-safe)). Не выполняется никаких преобразований при чтении/записи, байты читаются/пишутся, как есть. Используется при работе с нетекстовыми данными (например, JPG-рисунок).

Рассмотрим чтение строки
...
- в UNIX (GNU/Linux, Mac OS X):

  ```
  binary: ЗС 3F 70 68 70 0А
  text:   ЗС 3F 70 68 70 0А
  ```

- Microsoft Windows:

  ```
  binary: ЗС 3F 70 68 70 0D 0А
  text:   ЗС 3F 70 68 70 0А
  ```
