...
```php
var_dump( true ? 0 : true ? 1 : 2 )  # int(2)
var_dump( (true ? 0 : true) ? 1 : 2) # int(2)
```
...
[Ассоциативность тернарного оператора отличается от C/C++](#приоритет-и-ассоциативность-оператора).

Можно строить цепочки, возвращается первое значение, которое приводится к `true`:
...
Оператор `??` (или объединение с *null*, *null coalescing*) является синтаксическим сахаром:
...
<u>Список аргументов переменной длины</u>

Функция может быть определена с переменным числом аргументов с помощью символа `...`. Аргументы собираются в `array`:
...
Многоточие `...` можно использовать при вызове функции, чтобы распаковать `iterable` в список аргументов: Можно использовать как замену `call_user_func_array()`:
