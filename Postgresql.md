...
*Column*'у можно назначить *default value*. Когда добавляется новая *row* и каким-то её *column*'ам не присваиваются *value*'s, эти *column*'ы будут заполнены соответствующими *default value*'s. Также команда управления данными может явно указать, что *column*'у должно быть присвоено *default value*, без необходимости знать это значение. (Подробнее *data manipulation command*'ы описаны в [Главе 6](https://postgrespro.ru/docs/postgresql/9.6/dml).)

Если никакого *default value* не объявлено явно, им считается `NULL` *value*. Обычно это имеет смысл, так как можно считать, что `NULL` *value* представляет неизвестные данные. 

В том случае, если для *column* установлен *not-null constraint* и не указано *default value*, при добавлении новой *row* без явного указания *value* будет выдана ошибка:
...
[23502] ERROR: null value in column "b" violates not-null constraint
Подробности: Failing row contains (1, null).
```

В *table definition*, *default value*'s указываются после *column data type*. Например:
...
*Primary Key Constraint* означает, что образующий его *column* или группа *column*'s может быть использована как *unique identifier* для строк в таблице. Для этого требуется, чтобы значения были одновременно *unique* и НЕ `NULL`. 

Следующие две *table*'s позволяют вставлять одни и те же данные, но они не эквивалентны. 
...
При добавлении *primary key* автоматически создаётся *unique B-tree index* для *column* или группы *column*'s, перечисленных в *primary key*, и данные столбцы помечаются как `NOT NULL`.
...
**Таблица 8.19. Boolean Data Type**
...
Типы JSON предназначены для хранения данных JSON (JavaScript Object Notation) согласно стандарту [RFC 7159](https://tools.ietf.org/html/rfc7159). Такие данные можно хранить и в типе `text`, но типы JSON лучше тем, что проверяют, соответствует ли вводимое значение формату JSON. Для работы с ними есть также несколько *JSON-specific function*'s и *operator*'ов; смотрите [Section 9.16](https://www.postgresql.org/docs/current/functions-json.html).

В Postgres Pro имеются два *type*'s для хранения данных JSON: `json` и `jsonb`. Для реализации эффективного механизма *query* к этим *data type*'s в Postgres Pro также имеется `jsonpath` *data type*, описанный в [Section 8.14.7](https://www.postgresql.org/docs/current/datatype-json.html#DATATYPE-JSONPATH).

`json` и `jsonb` *data type*'s принимают на *input* *почти* одинаковые наборы *value*'s, а отличаются они главным образом с точки зрения эффективности. `json` *data type* сохраняет точную копию *input text*'а, который *function*'s обработки должны *reparse* заново при каждом *query execution*, тогда как `jsonb` *data* сохраняются в *decomposed binary format*, что несколько замедляет *input* из-за дополнительного *conversion overhead*, но значительно ускоряет обработку, не требуя многократного *text reparsing*. Кроме того, `jsonb` поддерживает *indexing*, что тоже может быть очень полезно.

Так как `json` *type* сохраняет точную копию *input text*, он сохраняет семантически незначащие пробелы (*white space*) между *token*'s (элементами), а также порядок *key*'s в JSON *object*'ах. И если *JSON object* внутри *value* содержит один и тот же *key* несколько раз, этот *type* сохранит все пары *key/value*. (*Processing function*'s будут считать последнюю пару как действительную (*operative*)). Наоборот, `jsonb` , не сохраняет пробелы (*white space*), не сохраняет порядок *object key*'s и значения с дублирующимися *object key*'s. Если в *input* оказываются дублирующиеся *key*'s, сохраняется только последнее *value*.

Для большинства приложений предпочтительнее хранить данные JSON в типе `jsonb` (если нет особой необходимости, например, важен исходный порядок *object key*'s).

TODO!!!

При конвертации текстового *JSON input*  в тип `jsonb`, примитивные *type*'s, описанные в RFC 7159, по сути маппятся в *native PostgreSQL type*'s как показано в [Таблице 8.23](https://postgrespro.ru/docs/postgrespro/15/datatype-json#JSON-TYPE-MAPPING-TABLE). Таким образом, к содержимому типа `jsonb` предъявляются некоторые дополнительные требования, продиктованные ограничениями представления *underlying data type*, которые не распространяются ни на `json` *type*, ни на формат JSON вообще. В частности, `jsonb` *type* не принимает числа, выходящие за диапазон PostgreSQL `numeric` *data type*, тогда как с `json` такого ограничения нет. Такие ограничения, накладываемые реализацией, допускаются согласно RFC 7159. Однако на практике такие проблемы более вероятны в других реализациях (что такое другие реализации?), так как обычно примитивный тип JSON `number` представляется в виде числа с плавающей точкой двойной точности IEEE 754 (что RFC 7159 явно признаёт и допускает). При использовании JSON в качестве формата обмена данными с такими системами следует учитывать риски потери точности чисел, хранившихся в Postgres Pro.

И напротив, как показано в таблице, есть некоторые ограничения в *input format*'е для *JSON primitive type*'s, не актуальные для соответствующих *PostgreSQL type*'s.

Таблица 8.23. *JSON Primitive Type*'s и соответствующие им типы *PostgreSQL Type*'s

| *JSON primitive type* | *PostgreSQL type* | Замечания                                                    |
| --------------------- | ----------------- | ------------------------------------------------------------ |
| `string`              | `text`            | `\u0000` не допускается, т.к. представляет символ, который отсутствует в кодировке базы |
...
| `null`                | (нет)             | `NULL` в SQL имеет другой смысл (как тип отсутствует)        |

### 8.14.1. JSON Input and Output Syntax

*Input/output syntax* для *JSON data type*'s соответствует стандарту RFC 7159.

Примеры допустимых *expression* с типом `json` (или `jsonb`):

- Простое скалярное/примитивное *value*

  Простыми *value* могут быть *number* (числа), *quoted string*, *true*, *false* или *null*.

  ```sql
  SELECT '5'::json;
  ```

- *Array* из нуля и более элементов (элементы могут быть разных типов):

  ```sql
  SELECT '[1, 2, "foo", null]'::json;
  ```

- *Object*, содержащий пары *key*'s и *value*'s

  Заметьте, что *object key* — должны быть всегда *quoted string*

  ```sql
  SELECT '{"bar": "baz", "balance": 7.77, "active": false}'::json;
  ```

- *Array* и *object* могут вкладываться могут вкладываться друг в друга без ограничений:

  ```sql
  SELECT '{"foo": [true, "bar"], "tags": {"a": 1, "b": null}}'::json;
  ```

Как было сказано ранее, если взять *JSON value* и привести к одному из *JSON typ*'ов:

- `json` – выводит тот же текст, что поступил на *input*
...
- `jsonb` – не сохраняет семантически незначащие детали, такие как пробелы, порядок элементов в структуре.
...
Стоит отметить одну семантически незначительную деталь: в `jsonb` числа выводятся по правилам *underlying type* `numeric`. На практике это означает, что числа, указанные в `E` *notation*, будут выведены без неё, например:
...
Как видно из этого примера, `jsonb` сохраняет конечные нули дробного числа, хотя они и не имеют семантической значимости, в частности для проверки на *equality* (`=`).

Список *built-in function* и *operator*'ов, позволяющих создавать и обрабатывать *JSON value*'s, приведён в [Разделе 9.16](https://postgrespro.ru/docs/postgrespro/15/functions-json).

### 8.14.2. Построение *JSON Document*'ов

Представлять данные в JSON можно гораздо более гибко, чем в традиционной *relational data model*, что очень привлекательно там, где нет жёстких условий. И оба этих подхода вполне могут сосуществовать и дополнять друг друга в одном *application*. Однако даже для *application*, которым нужна максимальная гибкость, рекомендуется, чтобы *JSON document*'ы имели некоторую фиксированную структуру. Эта структура обычно не навязывается жёстко (хотя можно декларативно диктовать некоторые бизнес-правила), но когда она предсказуема, становится гораздо проще писать *query*'s, которые извлекают полезные данные из набора «*document*'ов» (*datum*'s) в *table*.

*JSON data*, как и *data* любых других *type*, хранящихся в *table*, находятся под контролем механизмов *concurrency-control*. Хотя хранить большие *document*'ы вполне возможно, не забывайте, что при любом *update* выполняется *acquires* для *row-level lock* (блокировка на уровне строки)  для всей *row*. Поэтому для уменьшения конфликтов *lock*'ов среди *updating transaction*'s, стоит ограничить размер *JSON document*'ов до разумных пределов. В идеале каждый *JSON document* должен собой представлять атомарный *datum* (информационный блок), который, согласно бизнес-логике, нельзя разделить на меньшие *datum*'s, которые могут изменяться независимо.

### 8.14.3. Проверки на *containment* (вхождение) и *existence* (существование) `jsonb`

Различные *containment* и *existence operator*'ы, а также все другие *JSON operator*'ы и *function*'s документированы в [Section 9.16](https://www.postgresql.org/docs/current/functions-json.html).

#### Проверка на *containment*

Проверка *containment* — важная особенность `jsonb` *type*, не имеющая аналога для `json` *type*. Эта проверка определяет, *contain* ли один документ `jsonb` в другой. В следующих примерах возвращается `true` (кроме упомянутых исключений):

- Простые *scalar/primitive values contain* только одно идентичное *value*:
...
- *array*.
  ...
  Общие принципы: порядок *array element*'ов для проверки не имеет значения, а повторяющиеся *array element*'ы считаются только один раз.

  - *Array contain* в другой *array*, если он содержит какие-либо *element*'ы этого *array*:

    ```sql
    SELECT '[1, 2, 3]'::jsonb @> '[1, 3]'::jsonb;
    ```

  - Порядок *array element*'ов не важен:

    ```sql
    SELECT '[1, 2, 3]'::jsonb @> '[3, 1]'::jsonb;
    ```

  - *array element*'ы могут повторяться, по факту сравниваются уникальные *element*'ы в обоих *array*:
    ...
    ```sql
    SELECT '[1, 2, 2, 3]'::jsonb @> '[1, 2]'::jsonb;
    ```

  - *Array* слева – НЕ *contain* – *array* справа, хотя в него и вложен такой *array*:
  ...
  - Но если добавить уровень вложенности, проверка на *containment* возвращает `true`:

    ```sql
    SELECT '[1, 2, [1, 3]]'::jsonb @> '[[1, 3]]'::jsonb;
    ```

  - в качестве особого исключения  – *array contain* примитивное *value*, которое является его *element*'ом:

    ```sql
    SELECT '["foo", "bar"]'::jsonb @> '"bar"'::jsonb;
    ```

  - но исключение действует только в одну сторону, т.е. примитивное *value* НЕ *contain* *array*:
...
- *object*.
  ...
  Общий принцип проверки – *object*-справа должен соответствовать *object*'у-слева, содержащему его, по структуре и данным, возможно, после исключения из *object*'а-слева лишних *array element*'ов или *key/value pair*. 

  - *Object* слева – *contain* – *object* с одной парой справа:

    ```sql
    SELECT '{"product": "PostgreSQL", "version": 9.4, "jsonb": true}'::jsonb @> '{"version": 9.4}'::jsonb;
    ```

  - Аналогично *array*, *object* слева – НЕ *contain* – *object* справа, хотя он и является вложенным *object*'ом:
  ...
  - для проверки вложенного *object*'а можно использовать конструкции:

    - проверка, что содержится *key* с некоторым *object*'ом в качестве *value* – в качестве *value* использовать пустой *object*:

      ```sql
      SELECT '{"foo": {"bar": "baz"}}'::jsonb @> '{"foo": {}}'::jsonb; -- true
      SELECT '{"foo": "bar"}'::jsonb @> '{"foo": {}}'::jsonb; -- false, для не object'ов не работает
      SELECT '{"foo": {"bar": {"baz" : "abc"}}}'::jsonb @> '{"foo": {"bar": {}}}'::jsonb; -- true, работает для вложенных object'ов
      ```

    - для проверки, что содержится вложенный *object-value*, необходимо описать полный порядок вложенности:

      ```sql
      SELECT '{"foo": {"bar": "abc", "baz": "cde"}}'::jsonb @> '{"foo": {"bar": "abc"}}'::jsonb;
      ```

#### Проверка на *existence*

Для `jsonb` *type* введён также *existence* *operator*, который является вариацией на тему *containment*: он проверяет, является ли *string* (только *string*!!!!, заданная в виде `text` *value*) – *object key* или *array element*'ом на верхнем уровне `jsonb` *value*. В следующих примерах возвращается `true` (кроме упомянутых исключений):

- *array*

  - *string exist*, если она соответствует *array element*'у:
...
- *object*
  ...
  - *string exist*, если она соответствует *object key*:

    ```sql
    SELECT '{"foo": "bar"}'::jsonb ? 'foo';
    ```

  - проверить *existance* для *object value* нельзя:

    ```sql
    SELECT '{"foo": "bar"}'::jsonb ? 'bar';  -- false
    ```

  - как и *containment*, *existence* определяется на верхнем уровне:
...
- *string*
  ...
  - *string exist*, если она соответствует примитивной *JSON string*:
...
*JSON object*'ы лучше чем *array* подходят для проверок на *containment* и *existence*, когда они включают множество *key* или *element*'ов, так как, в отличие от *array*, *object*'ы внутри оптимизируются для поиска, и поиск не будет линейным.

#### Работа с вложенными object'ами

##### Containment

Так как *JSON containment* допускает учет вложенности, правильно написанный *query* может заменить явный *select* для внутренних *object*'ов. Например, предположим, что у нас есть `doc` *column*, содержащий *object*'ы на верхнем уровне, и большинство этих *object*'ов содержит `tags` *field*'s, которые содержат *array* с вложенными *object*'ами. 

Этот *query* найдёт *row*'s, в которых в `tags` *array* вложены *object*'ы `{"term":"paris"}` и `{"term":"food"}`, и при этом пропустит те, где *object*'ы находятся вне `tags` *array*:
...
Этого же результата можно добиться так:
...
Но данный подход менее гибкий (??? потому что требует обращаться в вложенному *field*) и часто также менее эффективный (почему? не используются индексы?)

##### Existance

Оператор *JSON existence* не поддерживает вложенность: он будет искать заданный *key* или *array element* только на верхнем уровне.

### 8.14.4. `jsonb` Indexing

Для эффективного поиска *key*'s или *key/value* пар в большом количестве `jsonb` *document*'ов (*datum*'s) можно успешно применять *GIN index*'ы. Для этого предоставляются два GIN “*operator class*'а”, предлагающие выбор между производительностью и гибкостью (ниже описаны преимущества по *performance*).

*Default GIN operator class* для `jsonb` (`jsonb_ops`) поддерживает *query*'s с *existence operator*'ами (`?`, `?|` и `?&`), *containment operator*'ом (`@>`) и `jsonpath` *match operator*'ами (`@?` и `@@`) (подробнее семантика, реализуемая этими *operator*'ами, описана в [Table 9.46](https://www.postgresql.org/docs/current/functions-json.html#FUNCTIONS-JSONB-OP-TABLE)). Пример создания *index*'а с этим *operator class*'ом:

```sql
CREATE INDEX idxgin ON api USING GIN (jdoc);
```

*Non-default GIN operator class* `jsonb_path_ops` не поддерживает *existence operator*'ы (`?`, `?|` и `?&`), но поддерживает *containment operator* (`@>`) и `jsonpath` *match operator*'ы (`@?` и `@@`). Пример создания *index*'а с этим *operator class*'ом:

```sql
CREATE INDEX idxginp ON api USING GIN (jdoc jsonb_path_ops);
```

Рассмотрим пример *table*, в которой хранятся *JSON document*'ы, получаемые от стороннего *web service*, с документированным определением схемы. Типичный *document*:
...
Мы сохраняем эти *document*'ы в `api` *table*, в `jdoc` *column* c `jsonb` *typ*'ом. Если по этому *column* создаётся *GIN index*, он может применяться в *query*:

```sql
-- Найти document'ы, в которых key "company" имеет value "Magnafone"
SELECT jdoc FROM api WHERE jdoc @> '{"company": "Magnafone"}';
```

Однако в следующем *query* он не будет использоваться, потому что, несмотря на то, что `?` *operator* — индексируемый, в *query* этот *operator* применяется не к индексированному `jdoc` *column* непосредственно, а к `tags` *field* (*existence operator* не поддерживает вложенность):

```sql
-- Найти document'ы, в которых ключ "tags" содержит ключ или элемент массива "qui"
SELECT jdoc->'guid', jdoc->'name' FROM api WHERE jdoc -> 'tags' ? 'qui';
```

Если в этом *query* необходимо задействовать *index*:

- можно построить *index* прямо по `tags` *field* (индекс выражения, *expression index*)

  ```sql
  CREATE INDEX idxgintags ON api USING GIN ((jdoc -> 'tags'));
  ```

  Если такой *query* для *array element*'ов в `"tags"` *key* выполняется часто, имеет смысл определить такой *index*.

  Теперь  `WHERE` *clause* `jdoc -> 'tags' ? 'qui'` применит индексируемый `?` *operator* к индексируемому *expression* `jdoc -> 'tags'` (подробнее об *expression index*'ах можно узнать в  [Section 11.7](https://www.postgresql.org/docs/current/indexes-expressional.html))

- можно использовать вместо *existence operator* – *containment operator*:

  ```sql
  -- Найти document'ы, в которых key "tags" содержит array element "qui"
  SELECT jdoc->'guid', jdoc->'name' FROM api WHERE jdoc @> '{"tags": ["qui"]}';
  ```

  Этот *query* может использовать простой *GIN index* по `jdoc` *column*. Но заметьте, что такой *index* будет хранить копии всех *key* и *value* в `jdoc` *column*, тогда как *expression index* из предыдущего примера хранит только данные из *array* с `tags` *key*. Хотя подход с простым *GIN index*'ом гораздо более гибкий (так как он поддерживает *query* по любому *key*), конкретный *expression index* скорее всего будет меньше и быстрее, чем простой *GIN index*.

- простой *GIN index* также поддерживает `@?` и `@@` *operator*'ы, которые сопоставляют `jsonpath` с данными. Примеры:
  ...
  Для `@?` и `@@`  *operator*'ов *GIN index* извлекает из `jsonpath` *pattern*'а предложения вида `accessors_chain = constant` (`цепочка_обращения = константа`) и выполняет поиск в *index*'е по *key* и *value* из этих предложений. Цепочка обращения может включать указания обращения `.*`ключ`*`, `[*]` и `[*`индекс`*]`. Класс операторов `jsonb_ops` также поддерживает указания `.*` и `.**`, которые не поддерживает класс операторов `jsonb_path_ops`.



### 8.15. Arrays

PostgreSQL позволяет определять *column*'s для *table* как *multidimensional array* (многомерный массив) с *variable-length*. Элементами *array* могут быть любые *built-in* или *user-defined base type, enum type, composite type, range type, or domain*. 

#### 8.15.1. Declaration of Array Types

Чтобы проиллюстрировать использование *array type*, мы создадим такую *table*:
...
Как показано, для объявления *array data type* к названию типа элементов добавляются квадратные скобки (`[]`). Показанная выше команда создаст *table* `sal_emp` с *column*'s типов `text` (`name`), *one-dimensional array* с элементами типа `integer` (`pay_by_quarter`), представляющий квартальную зарплату работников, и *two-dimensional array* с элементами `text` (`schedule`), представляющий недельный график работника.

Команда `CREATE TABLE` позволяет также указать точный размер *array*, например так:
...
Однако текущая реализация игнорирует все указанные размеры для *array*, т. е. поведение аналогично *array* с неуказанным размером.

Текущая реализация также не ограничивает число размерностей. Все элементы *array* считаются одного типа, вне зависимости от его размера и числа размерностей. Поэтому явно указывать число элементов или размерностей в команде `CREATE TABLE` имеет смысл только для документирования, на механизм работы с *array* это не влияет.

Для объявления one-*dimensional array* можно применять альтернативную запись с ключевым словом `ARRAY`, соответствующую стандарту SQL. Столбец `pay_by_quarter` можно было бы определить так:

```sql
    pay_by_quarter  integer ARRAY[4],
```

Или без указания *array size*:

```sql
    pay_by_quarter  integer ARRAY,
```

Заметьте, что в любом случае PostgreSQL не накладывает ограничения на фактический *array size*.
...
### 9.27.7. Functions для управления Database Object'ами

*Function*'s, показанные в [Таблице 9.97](https://postgrespro.ru/docs/postgrespro/15/functions-admin#FUNCTIONS-ADMIN-DBSIZE), вычисляют *disk space*, который занимают *database object*'ы, и помогают воспринять и проанализировать информацию об используемом объёме. Результаты типа `bignit` представляют значения в *byte*'s. Если в *function*'s передаётся OID (object ID???), не соответствующий существующему *object*'у, они возвращают `NULL`.

| Function</br>&emsp;Description                               |
| ------------------------------------------------------------ |
| `pg_table_size` ( `regclass` ) → `bigint`<br/>&ensp;Вычисляет *disk space*, который занимает данная *table*, за исключением *index*'ов, либо *index* |

Вышеописанные *functiion*'s, работающие с *table*'s или *index*'s, принимают аргумент типа `regclass`, который представляет собой просто *table OID* или *index OID* в  `pg_class` system catalog. Однако вам не нужно вручную вычислять OID, так как `regclass` *data type's input converter* может сделать это за вас. За подробностями обратитесь к [Разделу 8.19](https://postgrespro.ru/docs/postgrespro/15/datatype-oid).
...
*Hash index*'ы хранят *32-bit hash code*, полученный из значения *index*'ированного *column*, поэтому *hash index*'ы работают только с простыми условиями равенства `=`. *Query planner* может применить *hash index*, только если *index*'ированный *column* участвует в сравнении с оператором равенства `=`. Создать такой *index* можно следующей командой:

```
CREATE INDEX имя ON таблица USING HASH (столбец);
```

## 11.5. Объединение нескольких *index*'ов

При одиночном *index scan* могут обрабатываться только те *query clause*'s, в которых используются *index*'ированные *columns* (столбцы индекса) с *operator*'ом из его *operator class*'а и объединяет их `AND`. Например, для *index*'а `(a, b)` – *query condition*  `WHERE a = 5 AND b = 6` сможет использовать этот *index*, а *query* `WHERE a = 5 OR b = 6` — нет.

К счастью, Postgres Pro способен соединять несколько *index*'ов (и в том числе многократно применять один *index*), чтобы обрабатывать также случаи, когда недостаточно одиного *index scan*. Система может сформировать `AND` и `OR` *condition*'s за несколько *index scan*'s. Например, *query* `WHERE x = 42 OR x = 47 OR x = 53 OR x = 99` можно разбить на четыре *index scan*'s по `x`, по *scan*'ированию для каждого *query clause*. Затем результаты этих *scan*'ирований будут логически объединены OR и дадут конечный результат. Другой пример — если у нас есть отдельные *index*'ы по `x` и `y`, *query* `WHERE x = 5 AND y = 6` можно выполнить, применив *index*'ы для соответствующих *query clause*'s, а затем вычислив логическое AND для результатов *index scan*'ов, чтобы получить *result row*'s.

Выполняя объединение нескольких *index*'ов, система сканирует все необходимые *index*'ы и создаёт в памяти *bitmap*, расположения *table row*'s, которые удовлетворяют условиям каждого *index*'а. Затем все *bitmap*'ы объединяются операциями AND и OR, как того требуют условия в *query*. Наконец система обращается к соответствующим отфильтрованным *table row*'s и возвращает их данные. *Table row*'s выбираются в физическом порядке, а таком порядке они сохраняются в *bitmap*; это означает, что порядок сортировки в *index*'ах при этом теряется и в запросах с `ORDER BY` *clause* – *sort step* будет выполняться отдельно. По этой причине, а также потому, что каждое дополнительное *index scan*'ирование занимает дополнительное время, *planner* (планировщик) иногда выбирает обычное *index scan*'ирование, несмотря на то, что можно было бы использовать и дополнительные *index*'ы.

В большинстве приложений (кроме самых простых) полезными могут оказаться различные комбинации индексов, поэтому разработчик баз данных, определяя набор индексов, должен искать компромиссное решение. Иногда оказываются хороши составные индексы, а иногда лучше создать отдельные индексы и положиться на возможности объединения индексов. Например, если типичную нагрузку составляют запросы иногда с условием только по столбцу `x`, иногда только по `y`, а иногда по обоим столбцам, вы можете ограничиться двумя отдельными индексами по `x` и `y`, рассчитывая на то, что при обработке условий с обоими столбцами эти индексы будут объединяться. С другой стороны, вы можете создать один составной индекс по `(x, y)`. Этот индекс скорее всего будет работать эффективнее, чем объединение индексов, в запросах с двумя столбцами, но как говорилось в [Разделе 11.3](https://postgrespro.ru/docs/postgrespro/15/indexes-multicolumn), он будет практически бесполезен для запросов с ограничениями только по `y`, так что одного этого индекса будет недостаточно. Выигрышным в этом случае может быть сочетание составного индекса с отдельным индексом по `y`. В запросах, где задействуется только `x`, может применяться составной индекс, хотя он будет больше и, следовательно, медленнее индекса по одному `x`. Наконец, можно создать все три индекса, но это будет оправдано, только если данные в таблице изменяются гораздо реже, чем выполняется поиск в таблице, при этом частота запросов этих трёх типов примерно одинакова. Если запросы какого-то одного типа выполняются гораздо реже других, возможно лучше будет оставить только два индекса, соответствующих наиболее частым запросам.
...
Postgres Pro предоставляет следующие *index*'ы: B-tree, hash, GiST, SP-GiST, GIN, и BRIN. 

Если в команде присутствует `WHERE` *clause*, она создаёт *partial index*. Такой *index* содержит *entry*'s только для части *table*, обычно более полезной для индексации, чем остальная *table*. Например, если *table* содержит информацию об оплаченных и неоплаченных счетах, при этом последних сравнительно немного, но именно эта часть *table* наиболее востребована, то увеличить быстродействие можно, создав *index* только по этой части. Ещё одно возможное применение `WHERE` — добавить `UNIQUE`, обеспечить уникальность в подмножестве *table*. Подробнее это рассматривается в [Разделе 11.8](#11-8-partial-index).

*Expression* в `WHERE` *clause* может ссылаться только на *column*'s в *underlying table*, но не обязательно ограничиваться теми, по которым строится *index*. В настоящее время в `WHERE` также нельзя использовать *subquery* и *aggregate expression*. Это же ограничение распространяется и на *expression* в *index field*.
...
Создание *hash index*'а:

```sql
CREATE INDEX IF NOT EXISTS forms_buyer_id_idx ON forms USING hash(buyer_id);
```
...
Опциональное `ON CONFLICT` *clause* задаёт действие, заменяющее возникновение ошибки при нарушении *arbiter unique constraint* или *exclusion constraint* (*arbiter* ниже описан). Для каждой отдельной *row*, предложенной для *insert*, *insert* либо выполняется успешно, либо, если нарушается *constraint* или *index*, которые указываются в `conflict_target`, выполняется альтернативное `conflict_action`. Вариант `ON CONFLICT DO NOTHING` в качестве альтернативного действия просто отменяет *insert* для *row*. Вариант `ON CONFLICT DO UPDATE` изменяет существующую *row*, которая конфликтует с *row*, предложенной для *insert*.

TODO!!!

`ON CONFLICT DO UPDATE` гарантирует атомарный результат команды `INSERT` или `UPDATE`; при отсутствии внешних ошибок гарантируется один из двух этих исходов, даже при большой *concurrency*. Эта операция также известна как *UPSERT* — “UPDATE or INSERT”..

- *`conflict_target`*

  Определяет, для какого именно *conflict*'а в `ON CONFLICT` будет предпринято альтернативное действие, устанавливая *arbiter index*'s (решающие индексы). Это указание позволяет осуществить выбор *unique index* или явно задаёт имя *constraint*'а. Для `ON CONFLICT DO NOTHING` `conflict_target` может не указываться; в этом случае будут обрабатываться все *conflict*'ы с любыми *constraint*'ами (и *unique index*'ами). Для `ON CONFLICT DO UPDATE` `conflict_target` **должен** указываться.

- `conflict_action`

  Параметр *`conflict_action`*  задаёт альтернативное действие в случае конфликта. 

  Это может быть:

  - либо `DO NOTHING` (не делать ничего)
  - либо `DO UPDATE` *clause* (произвести изменение), в котором указываются точные детали операции `UPDATE`, выполняемой в случае конфликта. `SET` и `WHERE` *clauses* в `ON CONFLICT DO UPDATE` могут обращаться:
    - к существующей *row* по *table's name* или *alias*'у 
    - или к *row*, предлагаемым для добавления, используя специальную `excluded` *table*. 
...
*Insert* или *update* новых *distributor*'s при необходимости. Предполагается, что был определен *unique index*, который ограничивает значения, отображаемые в `did` *column*. Обратите внимание, что специальная `EXCLUDED` *table* используется, чтобы ссылаться на значения, изначально указанные для *insert*:
...
Вставить *distributor* или *nothing* для *row*'s, когда существует *EXCLUDED* *row* (*row* с соответствующим *constrained column* или *column*'s после срабатывания *row insert trigger*'s). Пример предполагает, что был определен *unique index*, который ограничивает *value*'s в `did` *column*:
