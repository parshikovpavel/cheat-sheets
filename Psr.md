...
Перечисленные ниже правила получены путем анализа кода наиболее популярных фреймворков (Symfony, YII, Laravel...). PSR-12 находится в разработке и должен заменить PSR-2.
...
Закрывающий тег ?> необходимо удалять из файлов, которые не содержат только РНР-код. Иначе, случайно забытый пробел после >? приведет к началу формирования тела ответа и невозможности вывода HTTP-заголовков.
...
В списке аргументов один пробел ставится только после каждой запятой. Пробел вначале и вконце списка аргуметов не допускается.
...
·      Как в методах класса, в списке аргументов и списке переменных один пробел ставится только после каждой запятой. Пробел вначале и вконце списка аргументов не допускается.
...
$instance = **new class extends** \Foo **implements**     \ArrayAccess,
     \Countable,
     \Serializable
 {
     *// Class content* };
...
- указывать стандарт кодирования при прогоне с помощью опции `--standard`
- нужно поменять стандарт кодирования по умолчанию
