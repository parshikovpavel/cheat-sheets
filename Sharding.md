...
- в какой-то момент шарды станут неравномерными и придется использовать дополнительное шардирование, например, по комментам.

- степень теплоты шардов со временем станет неравномерной.
...
Пример неудачного выбора ключа шардирования: для счетчика-метрики – ID сайта, т.к. большие сайты генерировали объем трафика на один ключ шардинга, больше чем мог обработать один сервер. Необходимо выбрать достаточно маленький ключ шардирования, чтобы он нагрузка на него влезала на один сервер. Например, что-то меньшее по размеру чем популярная страница в соцсети.

### Данные разделить на несколько кластеров
...
Пример: в системе ведется учет денег пользователей на счетах, для этого имеется таблица `account`:
...
\2. В каждый шард добавляем таблицу с логом примененных транзакций
...
В шарде SA запускаем локальную транзакцию (все изменения в одной транзакции!!!), в рамках которой выполняем:
...
\3. Аналогично в шарде SB выполняем локальную транзакцию, со вставкой в таблицу transaction_log и изменением в баланса таблице account.
...
Вместо служебного процесса можно использовать асинхронную обработку в Message broker, особенно в случае если шаги 2-4 невозможно выполнить из-за временной недоступности шарда SB. Для этого в очередь помещаются номера ожидающих выполнения транзакций, шаги 2-4 выполняет асинхронно consumer.
...
#### Без требования идемпотентности (из документации Mongo, CouchDB…)

Подробное описание http://rystsov.info/2012/09/01/cas.html. 
...
\3. dirty committed (updated IS NOT NULL AND operation_id IS NOT NULL, и в таблице transaction для операции state=’committed’) – транзакция зафиксирована, но служебный данные не очищены
...
\5. UPDATE в таблицах account для счетов accountA и accountB в шардах SA и SB с указанием balance = balance + updated, updated = null, operation_id=null
...
Шаги 5-6 выполняются уже после подтверждения изменений и применяют локальные изменения к account, а затем транакция удаляется. Шаги 5-6 могут быть также выполнены в момент чтения, как описано ниже.
...
a)   clean – строка корректна и дополнительных действий не требуется

b)  dirty committed – требуется выполнить шаги 5-6 предыдущего алгоритма, чтобы применить локальные изменения к account и transaction

c)   dirty uncommitted – возникла проблема, транзакцию нужно откатить (либо просигнализировать о проблеме и поместить в очередь задачу для попытки накатить повторно). Порядок отката транзакции:

​      i) UPDATE в таблице transaction с указанием state=’failed’

​      ii) UPDATE в таблицах account для счетов accountA и accountB в шардах SA и SB с указанием updated = null, operation_id=null

​      iii) Строка в итоге находится в состоянии до выполнения операции
