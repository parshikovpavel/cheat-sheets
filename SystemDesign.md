...
![image-20260717000441404](img/system_design/11.png)
...
https://devpractice.ru/c4-model-modeling-software-architecture/

Способы визуализации архитектуры:

- *UML*
- *Box & Arrows* - прямоугольники для представления сущностей и стрелки для указания связей между ними
- *C4* 
...
Модель *C4* предлагает похожий подход: четыре диаграммы, каждая из которых отвечает за свой уровень масштаба (детализации) проектируемой системы. 
...
![img](https://devpractice.ru/wp-content/uploads/2023/12/image-5.png)

### **Программная система**

![img](img/system_design/2.png)

Элемент, который определяет разрабатываемое нами решение, как некоторый один неделимый блок.

### **Контейнер**

![img](img/system_design/3.png)

![image-20260714023026487](img/system_design/4.png)

Это приложение или хранилище данных. Контейнер – это то, что запускается обособленно от других элементов общей программной системы. Например: веб-сервер, сервер приложений, база данных, очередь (Kafka и т.п.), файловая система, микросервис, мобильное приложение, single-page application и т.п.

### **Компонент**

![img](img/system_design/5.png)

Компоненты – это то из чего состоят контейнеры (наборы классов, отдельные пакеты (jar-, dll-библиотеки) и т.п.). Компоненты – это такие структурные элементы, которые не разворачиваются и не запускаются в виде отдельных сущностей.

## Виды диаграмм в *C4Model*

### Диаграмма Контекста

На самом высоком уровне располагается **диаграмма контекста**. На уровне диаграммы контекстов представляется то, как наша система взаимодействует с внешним миром. Представляет пользователей системы и тех, кто с ней взаимодействует.

На этом уровне используются абстракции:

![img](img/system_design/6.png)

Пример диаграммы контекста для сервиса автоматизации публикации сообщений в *Telegram*-каналы:

![img](img/system_design/7.png)
...
**Диаграмма контейнеров** – это второй уровень масштаба системы. Переходя на него, мы на один шаг спускаемся вглубь детализации нашей системы. Контейнеры, в данном случае, это не контейнеры в понимании единцы развертывания систем (типа *Docker*-контейнеров), это крупные строительные блоки нашей системы,  отдельно развертываемые приложения: веб-серверы, серверы-приложений, базы данных, мобильные приложения и т.п. То есть что-то такое, что можно брать и запускать отдельно от остальных частей системы. Если система разворачивается на одном сервере, то контейнеры можно воспринимать как отдельно запущенные процессы. 
...
![img](img/system_design/8.png)

Изобразим на диаграмме компонентов внутреннее устройство системы постинга сообщений.

![img](img/system_design/9.png)
...
**Диаграмма компонентов**. На ней отображается структура контейнеров, которые были выделены на уровне выше. Можно сделать несколько диаграмм, так чтобы каждая раскрывала детали каждого контейнера индивидуально. 

Нарисуем диаграмму компонентов для *Web API App*. Для *Web API App* пользователем является *Single-Page App*, именно он отправляет в него запросы по *REST API* согласно нашей задумке.

![img](https://devpractice.ru/wp-content/uploads/2023/12/image-9-1024x743.png)

### Диаграмма кода

**диаграмма кода**, на ней в виде *UML* (или подобной нотации) описываются важные, с точки зрения разработки/проектирования, аспекты системы.
...
- *high availability* (доступность, SLI и SLO), измеряется в девятках, определяет допустимый простой:

  ![image-20260717000829787](img/system_design/12.png)
...
- Как скоро ожидается наращивание мощностей? Какой масштаб планируется через 3 месяца, полгода, год?
- Как выглядит технологический стек компании? Какие существующие сервисы можно применить для упрощения архитектуры?

### Функциональные требования

Проговорить словами, что будет делать система!!!

- какие клиенты (android, ios, web, web)
...
```
размер_memory_cache = количество_entity * размер_entity * 20%
```
...
- Паттерн один сервис - одна база
- На этом шаге нужно посчитать размер *Entity*. 

- В БД хранится только метаинформация, большие данные (статика, тексты) выносятся в S3.
...
- между сущностями мало связей, данные не структурированы

- нужно хранить огромные объемы данных? легко выполняется масштабирование, встроенное шардирование
...
JWT токен нужен, чтобы не передавать учетные данные пользователя с каждым запросом к серверу.

Преимущества токенов:

- Токены доступа имеют ограниченный срок годности (обычно ~15 минут). Этого времени не достаточно, чтобы расшифровать надежный шифр. К тому времени, когда злоумышленник расшифрует запрос, токен уже выйдет из обращения и будет бесполезен.
- Токены не требуют обращения к БД для валидации (в отличии от авторизационных данных). Это позволяет снизить нагрузку на БД и ускорить обработку запросов сервером.

Токены могут передавать в:

- куках (устанавливает сервер, отправляет клиент)

- в теле запроса и ответа
...
Вертикальное масштабирование повышение производительности за счет добавления ресурсов процессора, памяти, диска и т. д. Сервис Amazon RDS (Relational Database Service) предлагает серверы БД с 24 Тб оперативной памяти. 
...
- единая точка отказа
- высокая стоимость, мощные серверы очень дорогие.

#### Горизонтальное масштабирование

Заключается в добавлении новых серверов. 
...
Можно релизить приложение в несколько датацентров и кластеров в них.Это обеспечивает доступность приложения в случае выхода одно из ДЦ из строя. В случае сбоя трафик может быть перенаправлен к ДЦ, в котором нет запрашиваемых данных. Распространенным решением является
репликация данных между несколькими ЦД. 
...
- собственная система статики (S3). Статика (картинки, видео) и динамика (php-скрипты) разносятся по разным серверам. Запросы к API-приложения, которые обрабатываются серверами динамики, небольшие по размеру и должны выполняться очень быстро. Запросы к статике, наоборот, тяжелые и для них не требуется значительная оперативность. Поэтому для них требуется разная конфигурация серверов по процессору и памяти. Если статика и динамика расположены на одном сервере, тяжелые запросы к статике начнут тормозить легкие оперативные запросы к API.
- облачное хранилище (Amazon S3), для того чтобы использовать готовую реализацию хранения и обработки статики.
...
CDN — это сеть географически распределенных серверов, которая используется для доставки статического содержимого. Для пользователя подбирается ближайший CDN. Серверы CDN кэшируют такие статические файлы, как изображения, видео, CSS, JavaScript и т. д. Можно обойтись своей системой хранения статики.

CDN запрашивает файл у серверов статики, а затем кэшировать его локально и передавать запрашивающему пользователю.

Файлы можно удалять из CDN до истечения их срока годности:

- С помощью API-интерфейсов, предоставляемых поставщиками CDN;

- использовать версионирование image.png?v=2.
...
Очередь сообщений — это *durability* компонент, который поддерживает асинхронное взаимодействие. *Producer*'ы, создают сообщения и публикуют их в очереди. *Consumer*'ы потребляют сообщения. 

Очереди для того, чтобы обрабатывать фоновые задачи
...
*Producer*'ы публикуют в очереди сообщений задания, например, по обработке фотографий. *Consumer*'ы достают задания из очереди и выполняют асинхронную обработку. *Consumer*'ы и *producer*'ы могут масштабироваться независимо друг от друга. Когда очередь становится слишком большой, для сокращения времени обработки добавляются новые *consumer*'ы. 
...
- Предотвращение сбоев. Единая точка отказа (single point of failure, SPOF)  — это компонент, выход из строя которого приводит к прекращению работы всей системы.  Устраняются за счет избыточности компонентов, размещенных в разных ДЦ.
...
- количество новых твитов
- Доход, транзакции

## Гео распределенность

Пользователи с помощью geoDNS направляются к ближайшему центру обработки данных. Это называется географической маршрутизацией. geoDNS — это сервис, который сопоставляет доменные имена с IP-адресами в зависимости от местонахождения пользователя.
