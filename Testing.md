...
- Manual – ручные тесты, которые невозможно покрыть автотестами
- E2E тесты (end-to-end) – тесты с использование реальных сборок Front и Back и баз данных, тест через взаимодействие с элементами UI. 
  - Самые долгие автоматизированные тесты.
  - Очень нестабильны
  - Дают уверенность, что пользователь все видит правильно
...
Разработка через тестирование требует создания автоматизированных модульных тестов.

Модульное тестирование (unit testing) — процесс проверки работоспособности обособленного модуля (или группы модулей). Т.е. ключевой момент –изолировать отдельные модули и показать, что по отдельности эти модули работоспособны. Этот тип тестирования обычно выполняется программистами.
...
Компонентное тестирование client-side. Проверяют отдельные компоненты client-side с *mocked server-side*.
