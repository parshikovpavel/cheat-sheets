...
Потом вводишь текст, а для завершения нажимаешь Ctrl+D.
...
Берет строку (или набор строк), объединяет их в одну команду, а затем передает этой команде оболочку для выполнения. Проще говоря: она заставляет оболочку интерпретировать переданную строку как полноценную команду.
...
в Bash называется **подстановкой команд** (command substitution). Её суть в том, что оболочка сначала выполняет команду внутри скобок, а затем подставляет результат этой команды прямо в место, где была записана конструкция. 

Вы пишете `$(команда)` оболочка запускает `команда`, забирает весь её вывод и подставляет его туда, где была сама конструкция.
...
При выполнении этой строки оболочка запустит 

```bash
date
```

, возьмёт её вывод (например, 
