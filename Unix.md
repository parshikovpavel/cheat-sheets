...
Оптимальное значение `iowait` до 0.5%. Для серверов, отдающих статику скорее всего этот параметр будет выше. Если `iowait > 10 %`, то нужно решать проблемы с IO. 
