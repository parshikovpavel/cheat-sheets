...
Для создания постоянных *environment variables*, нужно определить эти переменные в файлах конфигурации *bash*. Эти файлы конфигурации на самом деле являются скриптами, поэтому в них возможна любая *bash* команда (в том числе `export`).

*Environment variables* задаются в формате:
...
Можно создавать такие типы постоянных *environment variables*:

- для конкретного пользователя, которые загружаются каждый раз когда он входит в систему. С помощью файла `~/.bash_profile` (в Linux `~/.bashrc`).

- для всех пользователей, которые загружаются при каждом входе в систему. Возможно использовать несколько файлов, которые загружаются в таком порядке: `/etc/profile`, `/etc/bashrc`, `/etc/bashrc_Apple_Terminal`.
