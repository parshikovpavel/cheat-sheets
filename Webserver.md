...
Максимальное количество одновременно обрабатываемых соединений равно worker_processes*worker_connections.
...
#### Модуль логгирования (журнальные файлы)
...
Указывает куда пишутся сообщения об ошибках. Первый параметр задаёт файл, который будет хранить лог. Второй параметр определяет уровень лога и может принимать одно из следующих значений: debug, info, notice, warn, error, crit, alert или emerg. Уровни лога перечислены в порядке возрастания важности. При установке определённого уровня в лог попадают все сообщения указанного уровня и уровней большей важности.

Если директивы указана на уровне http и на уровне server, то в глобальный журнальный файл помещаются ошибки только уровня сервера (например, проблемы с запуском виртуального хоста), а проблемы на уровне виртуального хоста, например не интерпретируются файлы РНР,  пишутся в журнальный файл конкретного виртуального хоста.
